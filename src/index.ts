...
async function main(): Promise<void> {
  ...
  const deps: Deps = {
    ...
    coach:
      config.coachLlm === "anthropic"
        ? new AnthropicCoach(config)
        : config.coachLlm === "litellm"
          ? new OpenAICompatibleCoach(config)
          : new FakeCoach(),
  ...
  // Optionally serve the buyer onboarding page alongside the MCP (WEB_SERVE=1).
  if (config.webServe) startWebServer(deps);
